...
#else
        //if (mkdir(curr_item, DIRMODE) == -1) {    // gluster
        if (glfs_mkdir(fs, curr_item, DIRMODE) == -1) {
          FAIL("unable to create directory");
        }
#endif
      /*
...
#else
        //if (rmdir(curr_item) == -1) {    // gluster
        if (glfs_rmdir(fs, curr_item) == -1) {
          FAIL("unable to remove directory");
        }
#endif
      }
 ...
 */
          ...
          //if ((fd = open(curr_item, O_RDWR)) == -1) {    // gluster
          if ((gfd = glfs_open(fs, curr_item, O_RDWR)) == NULL) {
            FAIL("unable to open file");
          }
#endif
        /*
 ...
 */
            ...
            //if ((fd = open(curr_item, O_CREAT|O_RDWR, FILEMODE)) == -1) {    // gluster
            if ((gfd = glfs_creat(fs, curr_item, O_CREAT|O_RDWR, FILEMODE)) == NULL) {
              FAIL("unable to create file");
            }                        
#endif
          /*
 ...
 */
            ...
            //if ((fd = creat(curr_item, FILEMODE)) == -1) {    // gluster
            if ((gfd = glfs_creat(fs, curr_item, O_CREAT|O_RDWR, FILEMODE)) == NULL) {
              FAIL("unable to create file");
            }
#endif
 ...
 */
          ...
          //if (write(fd, write_buffer, write_bytes) != write_bytes) {    // gluster
          if (glfs_write(gfd, write_buffer, write_bytes, 0) != write_bytes) {
            FAIL("unable to write file");
          }
#endif
        }
        ...
        if ( sync_file ) {
          ...
          //if ( fsync(fd) == -1 ) {    // gluster
          if (glfs_fsync(gfd, NULL, NULL) == -1) {
            FAIL("unable to sync file");
          }
        }
#endif
...
#else
          //if (unlink(curr_item) == -1) {    // gluster
          if (glfs_unlink(fs, curr_item) == -1) {
            FAIL("unable to unlink file");
          }
#endif
        }
...
#else
    //if (stat(item, &buf) == -1) {    // gluster
    if (glfs_stat(fs, item, &buf) == -1) {
      if (dirs) {
        if ( verbose >= 3 ) {
          fprintf( stdout, "V-3: Stat'ing directory \"%s\"\n", item );
          fflush( stdout );
...
#else
    //if ((fd = open(item, O_RDWR, FILEMODE)) == -1) {    // gluster
    if ((gfd = glfs_open(fs, item, O_RDWR)) == NULL) {
      FAIL("unable to open file");
    }
#endif
...
#else
      //if (read(fd, read_buffer, read_bytes) != read_bytes) {    // gluster
      if (glfs_read(gfd, read_buffer, read_bytes, 0) != read_bytes) {
        FAIL("unable to read file");
      }
#endif
    }
...
void show_file_system_size(char *file_system) {
  ...
  plfs_error_t plfs_ret;
#endif
  struct statvfs glfsbuf;    // gluster
...
#else
  //if (statfs(file_system, &status_buffer) != 0) {    // gluster
  if (glfs_statvfs(fs, file_system, &glfsbuf) == -1) {
    FAIL("unable to statfs() file system");
  }
#endif
...
#else
  //total_file_system_size = status_buffer.f_blocks * status_buffer.f_bsize;    // gluster
  //free_file_system_size = status_buffer.f_bfree * status_buffer.f_bsize;
  total_file_system_size = glfsbuf.f_blocks * glfsbuf.f_bsize;
  free_file_system_size = glfsbuf.f_bfree * glfsbuf.f_bsize;
#endif
...
#else
  //if (realpath(file_system, real_path) == NULL) {    // gluster
  if (glfs_realpath(fs, file_system, real_path) == NULL) {
    FAIL("unable to use realpath()");
  }
#endif
...
#else
      //if (mkdir(dir, DIRMODE) == -1) {    // gluster
      if (glfs_mkdir(fs, dir, DIRMODE) == -1) {
        FAIL("Unable to create directory");
      }
#endif
    }
...
#else
      //if (rmdir(dir) == -1) {    // gluster
      if (glfs_rmdir(fs, dir) == -1) {
        FAIL("Unable to remove directory");
      }
#endif
    }
...
#else
        //if (mkdir(temp_path, DIRMODE) == -1) {    // gluster
        if (glfs_mkdir(fs, temp_path, DIRMODE) == -1) {
          FAIL("Unable to create directory");
        }
#endif
      }
...
#else
        //if (rmdir(temp_path) == -1) {    // gluster
        if (glfs_rmdir(fs, temp_path) == -1) {
          FAIL("Unable to remove directory");
        }
#endif
      }
...
int main(int argc, char **argv) {
    ...
    fs = glfs_new("dht_vol");
    if (!fs) {
        printf("glfs_new: return NULL\n");
        return -1;
    }
    glfs_set_volfile_server(fs, "tcp", "127.0.0.1", 24007);
    glfs_set_logging(fs, "/dev/stderr", 1);
    gfret = glfs_init(fs);
    if (gfret) {
      FAIL("glfs_init error");
    }
#ifdef _HAS_PLFS
    pid = getpid();
    uid = getuid();
...
#else
    if (rank < path_count) {
      if (strcmp(testdirpath, "/") != 0) {
        if (glfs_access(fs, testdirpath, F_OK) != 0) {
          if (glfs_mkdir(fs, testdirpath, DIRMODE) != 0) {
            FAIL("Unable to create test directory path");
          }
        }
      }
    }
    //if ((rank < path_count) && access(testdirpath, F_OK) != 0) {    // gluster
      //if (mkdir(testdirpath, DIRMODE) != 0) {    // gluster
    /*
    if ((rank < path_count) && glfs_access(fs, testdirpath, F_OK) != 0) {
      if (glfs_mkdir(fs, testdirpath, DIRMODE) != 0) {
        fprintf(stderr, "mkdir(%s): %s\n", testdirpath, strerror(errno));
        FAIL("Unable to create test directory path");
      }
    }*/
#endif
...
#else
            //if ((rank < path_count) && access(testdir, F_OK) != 0) {    // gluster
                //if (mkdir(testdir, DIRMODE) != 0) {    // gluster
            if ((rank < path_count) && glfs_access(fs, testdir, F_OK) != 0) {
                if (glfs_mkdir(fs, testdir, DIRMODE) != 0) {
                    FAIL("Unable to create test directory");
                }
            }
#endif
...
#else
                //if ((rank < path_count) && access(testdir, F_OK) == 0) {    // gluster
                  //if (rmdir(testdir) == -1) {    // gluster
                if ((rank < path_count) && glfs_access(fs, testdir, F_OK) == 0) {
                  if (glfs_rmdir(fs, testdir) == -1) {
                    FAIL("unable to remove directory");
                  }
                }
#endif
