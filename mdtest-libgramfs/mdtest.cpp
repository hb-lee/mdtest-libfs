...
#define FAIL(msg) do {							\
    fprintf(stdout, "%s: Process %d(%s): FAILED in %s, %s: %s\n",	\
	    timestamp(), mdtest_rank, hostname, __func__,			\
	    msg, strerror(errno));					\
    fflush(stdout);							\
...
#define FAIL(msg) do {							\
    fprintf(stdout, "%s: Process %d(%s): FAILED at %d, %s: %s\n",	\
	    timestamp(), mdtest_rank, hostname, __LINE__,			\
	    msg, strerror(errno));					\
    fflush(stdout);							\
    MPI_Abort(MPI_COMM_WORLD, 1);					\
  } while(0)
...
int mdtest_mkdir(const char* path, mode_t mode) {
	LOG(INFO)<<"path="<<path<<" mode="<<mode;
	if (mdts != NULL && MDTS_stripe_on) {
		char buf[1024] = { 0 };
		sprintf(buf, "lfs mkdir -i %d %s", mdts->indexes[mdtest_rank % mdts->num],
				path);
		if (system(buf) != 0) {
			fprintf(stderr, "LFS mkdir unable to make directory");
			return MDTEST_FAILURE;
		}
	} else if (bmount->gramfs_mkdir(path,0777) < 0) {
...
int mdtest_accessdir(const char *path, int mode) {
	LOG(INFO)<<"Done";
	if (bmount->gramfs_access(path,mode) == 0) {
		LOG(INFO)<<"Done";
		return MDTEST_SUCCESS;
	}
	LOG(INFO)<<"Done";
...
void create_remove_items_helper(int dirs, int create, char* path,
	...
	LOG(INFO)<<__FUNCTION__<<"path="<<path<<" itemNum="<<itemNum;
	...
	for (i = 0; i < items_per_dir; i++) {
		if (dirs) {
			if (create) {
				if ((mdtest_rank == 0) && (verbose >= 3)
						&& ((itemNum + i) % ITEM_COUNT == 0
								&& (itemNum + i != 0))) {
				...
				if (bmount->gramfs_rmdir(curr_item) < 0) {
			...
			if (create) {
				...
				if (collective_creates) {
					...
					if (bmount->gramfs_open(curr_item, 0777|O_CREAT) < 0) {
					...
					if (shared_file) {
						...
						if ((bmount->gramfs_open(curr_item, FILEMODE|O_CREAT)) < 0) {
						...
						LOG(INFO)<<"bmount->gramfs_open:"<<"curr_item="<<curr_item<<" FILEMODE="<<(FILEMODE);
						if ((bmount->gramfs_open(curr_item, FILEMODE|O_CREAT)) < 0) {
							FAIL("unable to create file");
						}
					}
				...
				if (write_bytes > 0) {
					...
					if (bmount->gramfs_write(curr_item, write_buffer, write_bytes,
							0) < 0) {
				...
				if (bmount->gramfs_close(curr_item) < 0) {
				...
				if (!(shared_file && mdtest_rank != 0)) {
					if (bmount->gramfs_unlink(curr_item) < 0) {
...
void mdtest_stat(int random, int dirs, char *path) {
	...
	for (i = 0; i < stop; i++) {
		...
		if (dirs) {
			if (mdtest_rank == 0 && verbose >= 3 && (i%ITEM_COUNT == 0) && (i != 0)) {
		...
		if(dirs) {
			if (bmount->gramfs_statdir(item, buf) < 0) {
				if ( verbose >= 3 ) {
			...
			if (bmount->gramfs_stat(item,buf) < 0) {
				if (bmount->gramfs_stat(item,buf) < 0) {
					if ( verbose >= 3 ) {
...
void mdtest_read(int random, int dirs, char *path) {
	...
	for (i = 0; i < stop; i++) {
		...
		if (leaf_only) {
			item_num += items_per_dir
					* (num_dirs_in_tree
							- (unsigned long long) pow(branch_factor, depth));
		}
		...
		if (dirs) {
			...
			if (mdtest_rank == 0 && verbose >= 3 && (i % ITEM_COUNT == 0)
					&& (i != 0)) {
		...
		if (parent_dir > 0) {        //item is not in tree's root directory
			...
			/* still not at the tree's root dir */
			while (parent_dir > branch_factor) {
				parent_dir = (unsigned long long) ((parent_dir - 1)
						/ branch_factor);
		...
		if ((bmount->gramfs_open(item, FILEMODE|O_CREAT)) < 0) {
			FAIL("unable to open file");
		}

		LOG(INFO)<<"Done";
		LOG(INFO)<<"item="<<item<<" read_bytes="<<read_bytes;
		/* read file */
		if (read_bytes > 0) {
			std::string read_string(read_buffer);
			if (bmount->gramfs_read(item, read_string, read_bytes, 0) < 0) {
...
/* This method should be called by mdtest_rank 0.  It subsequently does all of
 the creates and removes for the other mdtest_ranks */
void collective_create_remove(int create, int dirs, int ntasks, char *path) {
	...
	/* mdtest_rank 0 does all of the creates and removes for all of the mdtest_ranks */
	for (i = 0; i < ntasks; i++) {
...
void directory_test(int iteration, int ntasks, char *path) {
	LOG(WARNING)<<__FUNCTION__<<" <<< Start >>>";
	int size;
	double t[5] = { 0 };
	char temp_path[MAX_LEN];
...
void file_test(int iteration, int ntasks, char *path) {
	...
	if (create_only) {
		...
		// create_remove I am going to make this an else statement now but needs further testing
			...
			LOG(INFO)<<__FUNCTION__<<"open(create) files Done";
		}
	}
	//printf("XXX mdtest_rank %d after create and write\n", mdtest_rank);
	fflush( stdout);
	...
	if (stat_only) {
		...
		LOG(INFO)<<__FUNCTION__<<"stat files Done";
	}
	...
	if (read_only) {
		...
		LOG(INFO)<<__FUNCTION__<<"read files Done";
	}
	...
	if (remove_only) {
		...
		LOG(INFO)<<__FUNCTION__<<"remove files Done";
	}
...
void valid_tests() {
	...
	if (unique_dir_per_task && shared_file && mdtest_rank == 0) {
	...
	/* check if more directory paths than mdtest_ranks */
	if (path_count > size) {
...
void show_file_system_size(char *file_system) {
	...
	if (bmount->gramfs_statfs(file_system,status_buffer) < 0) {
...
void create_remove_directory_tree(int create, int currDepth, char* path,
	...
	//  fprintf( stdout, "mdtest_rank: %d  Current Depth: %d\n", mdtest_rank, currDepth);
...
//      create_remove_directory_tree(create, ++currDepth, dir, ++dirNum);
		...
		for (i = 0; i < branch_factor; i++) {
			...
			if (!create) {
				...
				if (bmount->gramfs_rmdir(temp_path) < 0) {
					FAIL("Unable to remove directory");
				}
			}
...
int main(int argc, char **argv) {
	...
	MPI_Init(&argc, &argv);
	MPI_Comm_rank(MPI_COMM_WORLD, &mdtest_rank);
	...
	while (1) {
		...
		case 'M': { /* Auto fill the meta data target indexes */
			...
			/* Have mdtest_rank 0 figure out what MDTS are availible */
			if (mdtest_rank == 0) {
				char buf[1024];
				fflush(stdout);
			...
			/* The not mdtest_rank zero nodes */
			else {
		...
		case 'R':
			if (optarg == NULL) {
				...
				random_seed += mdtest_rank;
			} else {
				random_seed = atoi(optarg) + mdtest_rank;
			}
			break;
		case 's':
			stride = atoi(optarg);
	...
	if (depth <= 0) {
		...
		if (branch_factor < 1) {
			...
			num_dirs_in_tree = (1 - pow(branch_factor, depth + 1))
					/ (1 - branch_factor);
		}
	}
	if (items_per_dir > 0) {
	...
	if (random_seed > 0) {
		...
		while (n > 1) {
			...
			unsigned long long k = (unsigned long long) (((double) rand()
					/ (double) RAND_MAX) * (double) n);
	...
	if (path_count == 0) { /* special case where no directory path provided with '-d' option */
		...
		strcpy(testdirpath, filenames[mdtest_rank % path_count]);
	}
    bmount = new GramfsClient();
    std::string tmp;
    //tmp.append(testdirpath);
    //tmp.append("/Users/apple/Desktop/locofs/LocoFS/src/config.json");
    //tmp.append("../../test/scale.config");    // for one server
	tmp.append("../../test/gramfs.config");    // for multi-server
    bmount->gramfs_mount(tmp);
    //bmount->gramfs_mount(testdirpath);
    bmount->gramfs_mkdir("/",0777);

	/*   if directory does not exist, create it */
//	if (mdtest_rank < path_count) {
//
...
//	}
	...
	if (unique_dir_per_task) {
		...
		sprintf(base_tree_name, "mdtest_tree%s%d", file_dot, mdtest_rank);
	...
	for (i = first; i <= last && i <= size; i += stride) {
		...
		for (j = 0; j < iterations; j++) {
			...
			if (mdtest_rank < path_count) {
				LOG(INFO)<<"Done";
				LOG(INFO)<<"MDTEST_SUCCESS="<<MDTEST_SUCCESS;
				LOG(INFO)<<"mdtest_accessdir() "<<"testdir="<<testdir<<"  "<<"F_OK="<<F_OK;
			...
			if (create_only) {
				...
				if (unique_dir_per_task) {
					if (collective_creates && (mdtest_rank == 0)) {
						/*
				...
				MPI_Barrier(MPI_COMM_WORLD);
				endCreate = MPI_Wtime();
				summary_table[j].entry[8] = num_dirs_in_tree
			...
			if (mdtest_rank < i) {
				if (!shared_file) {
					sprintf(mk_name, "mdtest%s%d%s", file_dot,
							(mdtest_rank + (0 * nstride)) % i, file_dot);
					sprintf(stat_name, "mdtest%s%d%s", file_dot,
							(mdtest_rank + (1 * nstride)) % i, file_dot);
					sprintf(read_name, "mdtest%s%d%s", file_dot,
							(mdtest_rank + (2 * nstride)) % i, file_dot);
					sprintf(rm_name, "mdtest%s%d%s", file_dot,
							(mdtest_rank + (3 * nstride)) % i, file_dot);
				}
				if (unique_dir_per_task) {
					sprintf(unique_mk_dir, "%s%smdtest_tree%s%d%s0", testdir,
							dir_slash, file_dot, (mdtest_rank + (0 * nstride)) % i,
							file_dot);
					sprintf(unique_chdir_dir, "%s%smdtest_tree%s%d%s0", testdir,
							dir_slash, file_dot, (mdtest_rank + (1 * nstride)) % i,
							file_dot);
					sprintf(unique_stat_dir, "%s%smdtest_tree%s%d%s0", testdir,
							dir_slash, file_dot, (mdtest_rank + (2 * nstride)) % i,
							file_dot);
					sprintf(unique_read_dir, "%s%smdtest_tree%s%d%s0", testdir,
							dir_slash, file_dot, (mdtest_rank + (3 * nstride)) % i,
							file_dot);
					sprintf(unique_rm_dir, "%s%smdtest_tree%s%d%s0", testdir,
							dir_slash, file_dot, (mdtest_rank + (4 * nstride)) % i,
							file_dot);
					sprintf(unique_rm_uni_dir, "%s", testdir);
				}
				strcpy(top_dir, unique_mk_dir);
				LOG(INFO)<<"top_dir="<<top_dir;
				...
				if (dirs_only && !shared_file) {
					...
					LOG(INFO)<<"Done";
					LOG(INFO)<<"j="<<j<<" i="<<i<<" unique_mk_dir="<<unique_mk_dir;
			...
			if (remove_only) {
				...
				if ((mdtest_rank < path_count)
						&& bmount->gramfs_access(testdir, F_OK) == 0) {
					//if (( mdtest_rank == 0 ) && access(testdir, F_OK) == 0) {
					if (bmount->gramfs_rmdir(testdir) < 0) {
						FAIL("unable to remove directory");
					}
				}
			} else {
